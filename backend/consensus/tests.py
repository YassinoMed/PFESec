...
def make_vote(model_id: str, confidence: float, response: str = None,
    ...
    return ModelVote(
        ...
        weight=1,
        response=response if response is not None else f"Response from {model_id}",
...
def test_all_high_confidence():
    ...
    expected_score = (95.0 * 4 + 93.0 * 4 + 91.0 * 4) / (4 + 4 + 4)
    assert abs(result.global_score - expected_score) < 0.01
...
def test_single_above_threshold():
    ...
    assert abs(result.global_score - 92.0) < 0.01
    assert result.confidence_level == "High"
...
def test_none_above_threshold():
    ...
    assert result.total_retained == 1
    assert result.total_rejected == 2
    assert result.retained[0].weight == 1
    assert result.primary_model.model_id == "model_a"
...
def test_contradictory_responses():
    ...
    assert result.consensus_reached == False
    assert result.primary_model.model_id == "model_a"
    assert result.final_response == "PHISHING"
...
def test_empty_response_rejected():
    engine = ConsensusEngine()
    votes = [
        make_vote("model_a", 95.0, ""),
        make_vote("model_b", 91.0, "PHISHING"),
    ]
    result = engine.compute("test query", votes)
    assert result.total_retained == 1
    assert result.total_rejected == 1
    assert result.rejected[0].rejection_reason == "empty_response"
    assert result.primary_model.model_id == "model_b"
    print("  PASS: empty_response_rejected -> invalid vote excluded")
